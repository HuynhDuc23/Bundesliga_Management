...
export const getAllMatchTeams = async (req, res) => {
    try {
        const matchteams = await TeamMatch.find();
        if (!matchteams) {
...
export const getMatchTeamsByMatchId = async (req, res) => {
    try {
        ...
        const matchTeams = await TeamMatch.find({ match: ID_match });
        if (!matchTeams) {
...
export const getMatchTeamByIdTeam = async (req, res) => {
    try {
        const { id } = req.params;
        const matchteam = await TeamMatch.findOne({ team: id });
        if (!matchteam) {
...
export const createMatchTeam = async (req, res) => {
    try {
        ...
        const matchteam1 = new TeamMatch({
            match: matchId,
            team: teamId1,
            status: 0,
            score: 0,
        ...
        const matchteam2 = new TeamMatch({
            match: matchId,
            team: teamId2,
            status: 0,
            score: 0,
